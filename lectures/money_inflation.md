...
- a peculiar comparative stationary-state outcome connected with that stationary inflation rate: it asserts that inflation can be _reduced_ by running _higher_ government deficits, i.e., by raising more resources by printing money.
...
- it reverses the perverse dynamics by making the _lower_ stationary inflation rate the one to which the system typically converges
- a more plausible comparative dynamic outcome emerges in which now inflation can be _reduced_ by running _lower_ government deficits
...
\frac{m*{t+1}}{p_t} - \frac{m*{t}}{p*{t-1}} \frac{p*{t-1}}{p_t} = g
...
\bar R\_{\rm max} = \sqrt{\frac{\gamma_2}{\gamma_1}}
...
(\gamma*1 + \gamma_2) - \frac{\gamma_2}{\bar R*{\rm max}} - \gamma*1 \bar R*{\rm max}
...
Remember that there exist two steady-state equilibrium values $ R\_\ell < R_u$ of the rate of return on currency $R_t$.
...
Notice how sequences that start from $R_0$ in the half-open interval $[R_\ell, R_u)$ converge to the steady state associated with to $ R\_\ell$.
...
y*0 = \begin{bmatrix} m*{0} \cr p_0 \end{bmatrix} .
...
\Lambda = \begin{bmatrix} {R\_\ell}^{-1} & 0 \cr
0 & {R_u}^{-1} \end{bmatrix}
...
Q =\begin{bmatrix} Q*{11} & Q*{12} \cr
Q*{21} & Q*{22} \end{bmatrix}
...
p*0 = \overline p_0 \equiv Q*{21} Q*{11}^{-1} m*{0} ,
...
\frac{p\_{t+1}}{p_t} = {R_u}^{-1}, \quad t \geq 0
...
\lim*{t\rightarrow + \infty} \frac{p*{t+1}}{p*t} = {R*\ell}^{-1}.
...
Q^{-1} y*0 = y^*\_0 = \begin{bmatrix} y^**{1,0} \cr 0 \end{bmatrix} .
...
\begin{bmatrix} Q^{21} & Q^{22} \end{bmatrix} \begin{bmatrix} Q*{11}\cr Q*{21} \end{bmatrix} = 0
